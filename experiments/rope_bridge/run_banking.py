"""Run axor STRICT vs undefended on the banking suite inside ROPE's harness.

Usage:
  python -m axor_rope.run_banking --mode clean   --governed   [--limit-ut N]
  python -m axor_rope.run_banking --mode attack   --governed
  python -m axor_rope.run_banking --mode clean   --undefended
Env: OPENROUTER_API_KEY (mapped from OPEN_ROUTER_API_KEY by the shell wrapper).
"""
...
from axor_rope.bridge import (build_governed_pipeline, build_origin_pipeline,
                              build_undefended_pipeline)
from axor_rope.taxonomy import BANKING, BANKING_CHARGED, BANKING_ORIGIN

BENCH = "v1.2.2"
MODEL = os.environ.get("ROPE_AGENT_MODEL", "openai/gpt-4o-mini")
TAG = "gpt-4o-mini-2024-07-18-openrouter"   # must contain an AgentDojo model name
...
def main() -> int:
    ...
    ap.add_argument("--charged", action="store_true",
                    help="charge the consequence axis (update_password/user_info = CATASTROPHIC)")
    ap.add_argument("--origin", action="store_true",
                    help="origin config: integrity_default=context, integrity_sinks, NO allowlist")
    ...
    suite = get_suite(BENCH, "banking")

    cond = "governed" if args.governed else "undefended"
    if args.origin:
        cond += "-origin"
    elif args.charged:
        cond += "-charged"
    name = f"{TAG}-axor-{cond}"
    if args.governed and args.origin:
        pipe = build_origin_pipeline(llm, BANKING_ORIGIN, name=name)
    elif args.governed:
        tax = BANKING_CHARGED if args.charged else BANKING
        pipe = build_governed_pipeline(llm, suite, tax, name=name)
    else:
    ...
    print(f"[axor] banking {args.mode} {cond} model={MODEL} "
